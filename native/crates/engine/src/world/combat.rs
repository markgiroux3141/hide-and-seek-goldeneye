...
impl World {
    ...
    /// Whether the crosshair should be drawn this frame. Shown only while
    /// **aiming** (HUNT, RMB held) — matching GoldenEye's aim-mode reticle. Never
    /// in BUILD (the editor uses the face highlight for feedback, not a reticle).
    pub fn crosshair_visible(&self) -> bool {
        self.aiming
    }
    /// The weapon's static gun mesh, for one-time GPU upload at startup. `None` if
    /// the asset failed to load.
    pub fn gun_model(&self) -> Option<&GunModel> {
    ...
    pub fn muzzle_model(&self) -> Option<&GunModel> {
